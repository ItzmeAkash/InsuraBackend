...
from routes.utils import (
    extract_back_page_emirate,
    extract_front_page_emirate,
...
async def upload_pdf(file: UploadFile = File(...), user_id: str = Form(...)):
    ...
    if user_id not in user_states:
        user_states[user_id] = {"document_name": file}
...
async def upload_mulkiya_document(
    ...
    # Initialize user state if not already present
    if user_id not in user_states:
        user_states[user_id] = {"document_name": file.filename}

    # Get file extension and convert to lowercase
    file_extension = os.path.splitext(file.filename)[1].lower()

    # Validate file type
    valid_extensions = {"pdf": [".pdf"], "image": [".jpg", ".jpeg", ".png"]}
...
def get_pdf(document_name: str):
    ...
    if os.path.exists(pdf_path):
        return FileResponse(
            pdf_path, media_type="application/pdf", filename=f"{document_name}.pdf"
        )
    else:
...
def view_pdf():
    """
    View PDF documents for specific purposes like chronic conditions
    """
    pdf_path = f"pdf/Silver-AD 20% (160623) (6).pdf"
    if os.path.exists(pdf_path):
        return FileResponse(
            pdf_path,
            media_type="application/pdf",
            filename="Silver-AD 20% (160623) (6).pdf",
        )
    else:
